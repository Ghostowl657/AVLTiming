classdef Main_Wing
    ...
    methods
%% CONSTRUCTOR
        function obj=Main_Wing(B,S,Taper,Sweep,Airfoil,Dihedral,Incidence,Surfaces,Coord,Name) %% class constructor
            obj.b=B;
            obj.s=S;
            obj.taper=Taper;
            obj.sweep=Sweep;
...
%% BUILD AVL FILE
        function build_surface(obj,fid,Nchord,Nspan)
            surfaces=obj.surfaces;
            fprintf(fid,'#========================================== \n');
            fprintf(fid,'SURFACE \n');
            fprintf(fid,'%s \n',obj.name);
            fprintf(fid,'#Nchordwise Cspace Nspanwise Sspace \n');
            fprintf(fid,[num2str(Nchord) ' 1.0 ' num2str(Nspan) ' 1.0 \n']);
            fprintf(fid,'# \n');
